public class DamageManager
{
    ...
    float GetBaseDamage(AttackerStats atk)
    {
        return atk.attackType * atk.baseEscalator * atk.atqMultiplier * atk.multiplierDamage;
    }

    float GetResistenceMultiplier(AttackerStats atk, DefenderStats def)
    {
    ...
    float GetDefenseFactor(AttackerStats atk, DefenderStats def)
    {
        return (atk.level + 20) /
            ((def.level + 20) * (0.1f + def.defense - def.reduction) * (atk.level + 20));
    }

    float GetVulnerabilityFactor(AttackerStats atk, DefenderStats def)
    {
        return 1.0f - def.mitigation + atk.vulnerability;
    }

    float GetResistanceFactor(AttackerStats atk, DefenderStats def)
    {
